...
def process_taxii_to_misp(config, taxii_client, misp_client, stix_processor, redis_client=None, duplicate_checker=None, quality_filter=None):
    ...
    try:
        ...
        logger.info(f"TAXII connection verified. Collection: {connection_info.get('collection', {}).get('title', 'Unknown')}")
        ...
        batch_size = config.TAXII_BATCH_SIZE if hasattr(config, 'TAXII_BATCH_SIZE') else 10000
        rest_seconds = config.TAXII_REST_SECONDS if hasattr(config, 'TAXII_REST_SECONDS') else 5
        
        logger.info(f"📋 Batch configuration: {batch_size} objects per batch, {rest_seconds}s rest between batches")
        ...
        for batch_num, batch_objects in enumerate(taxii_client.get_all_objects_with_resource_management(
            ...
            logger.info(f"📦 Processing batch {batch_num} with {len(batch_objects)} objects...")
            
            # Create memory store for this batch
            memory_store, grouping_ids = taxii_client.extract_grouping_objects(batch_objects)
            ...
            if memory_store and grouping_ids:
                ...
                events_in_batch = process_batch_groupings(
                    memory_store, grouping_ids, stix_processor, misp_client, 
                    redis_client, duplicate_checker, quality_filter, config
                )
                
                events_pushed_count += events_in_batch
                logger.info(f"✅ Batch {batch_num}: {events_in_batch} events published to MISP")
            else:
            ...
            logger.info(f"📊 Progress: {total_objects} objects processed across {batch_num} batches")
        
        logger.info(f"🎯 Resource-managed processing completed: {total_objects} total objects, {events_pushed_count} events published")
...
def process_batch_groupings(memory_store, grouping_ids, stix_processor, misp_client, redis_client, duplicate_checker, quality_filter, config):
    """
    ...
    if redis_client and redis_client.is_connected():
        ...
        if filtered_count > 0:
            logger.info(f"Filtered out {filtered_count} already processed groupings, "
    ...
    for grouping_id in grouping_ids:
        ...
        try:
            ...
            if skipped_events:
                logger.info(f"🛡️  Quality filter skipped {len(skipped_events)} low-quality events for grouping '{grouping_id}'")
            
            if not filtered_events:
                logger.info(f"🚫 All events filtered out for grouping '{grouping_id}' - skipping MISP publication")
                continue
            
            logger.info(f"✅ Quality filter passed {len(filtered_events)}/{len(misp_events)} events for grouping '{grouping_id}'")
            ...
            for event in filtered_events:
                try:
                    ...
                    if duplicate_checker:
                        ...
                        logger.info(f"🔍 SIMPLE DUPLICATE CHECK: Checking event '{event.info if hasattr(event, 'info') else str(event)}'")
                        is_duplicate, duplicate_data = duplicate_checker.simple_misp_duplicate_check(
                            event.info if hasattr(event, 'info') else str(event)
                        )
                        
                        if is_duplicate:
                            logger.info(f"🚫 SIMPLE DUPLICATE DETECTED: Skipping event '{event.info if hasattr(event, 'info') else str(event)}'")
                            logger.info(f"   Existing event: ID={duplicate_data.get('id')}, Info='{duplicate_data.get('info')}'")
                            skip_event = True
                        else:
                            # Fallback to comprehensive duplicate check
                            logger.info(f"🔍 COMPREHENSIVE DUPLICATE CHECK: Checking event '{event.info if hasattr(event, 'info') else str(event)}'")
                            event_exists, existing_info = duplicate_checker.check_event_exists(
                                event_info=event.info if hasattr(event, 'info') else str(event),
                                grouping_id=grouping_id
                            )
                            
                            if event_exists:
                                logger.info(f"🚫 COMPREHENSIVE DUPLICATE DETECTED: Skipping event '{event.info if hasattr(event, 'info') else str(event)}': {existing_info}")
                                skip_event = True
                            else:
                                logger.info(f"✅ NO DUPLICATES FOUND: Event '{event.info if hasattr(event, 'info') else str(event)}' is unique - proceeding")
                    else:
                        ...
                        logger.info(f"🔍 FALLBACK DUPLICATE CHECK: Checking MISP database for event '{event_name}'")
                        
                        try:
                            # Simple MISP database check by event name
                            search_results = misp_client.misp.search_index(
                                eventinfo=event_name.strip(), 
                                limit=10,
                                pythonify=True
                            )
                                ...
                                for result in search_results:
                                    try:
                                        ...
                                        if isinstance(result_obj, dict):
                                            existing_name = result_obj.get('info', '').strip()
                                            # Check for exact match (case-insensitive)
                                            if existing_name.lower() == event_name.lower():
                                                logger.info(f"🚫 FALLBACK DUPLICATE DETECTED: Skipping duplicate event '{event_name}' (found: {existing_name})")
                                                skip_event = True
                            ...
                            if not skip_event:
                                logger.info(f"✅ FALLBACK DUPLICATE CHECK: No duplicate found for '{event_name}' - proceeding")
                            
                        except Exception as e:
                            logger.warning(f"⚠️  FALLBACK DUPLICATE CHECK FAILED: {e}. Proceeding with event creation.")
                    ...
                    # Add event to MISP
                    logger.info(f"📤 MISP CLIENT: Adding event '{event.info if hasattr(event, 'info') else str(event)}'")
                    misp_response = misp_client.add_event(event)
                    
                    if misp_response and 'Event' in misp_response and 'id' in misp_response['Event']:
                        event_id = misp_response['Event']['id']
                        event_uuid = misp_response['Event']['uuid']
                        
                        logger.info(f"Event added successfully to MISP: ID {event_id} (UUID {event_uuid})")
                        ...
                        if duplicate_checker:
                            ...
                            if grouping_obj:
                                if hasattr(grouping_obj, '_inner'):
                                    ...
                                    grouping_dict = dict(grouping_obj) if hasattr(grouping_obj, '__dict__') else {}
                        ...
                        misp_client.publish_event(event_id, alert=config.MISP_ALERT_ON_PUBLISH)
                        ...
                        if redis_client and redis_client.is_connected():
                            ...
                            if success:
                                logger.info(f"Marked grouping {grouping_id} as processed in Redis")
                            else:
                                logger.warning(f"Failed to mark grouping {grouping_id} as processed in Redis")
                        
                        events_pushed_count += 1
                    else:
                        logger.warning(f"Failed to add grouping '{grouping_id}' to MISP. Response: {misp_response}")
                
                except Exception as e:
                    logger.error(f"Error adding/publishing event for grouping '{grouping_id}': {e}", exc_info=True)
...
def main():
    ...
    try:
        ...
        logger.info(f"Max Events Per Run: {'Unlimited' if config.MAX_EVENTS_PER_RUN is None else config.MAX_EVENTS_PER_RUN}")
        ...
        logger.info(f"TAXII Request Timeout: {getattr(config, 'TAXII_REQUEST_TIMEOUT', 300)} seconds")
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Both MISP client initialization methods failed: {alt_error}")
                raise
        ...
        if config.REDIS_ENABLE_TRACKING:
            try:
                ...
                if redis_client.is_connected():
                    logger.info("Redis client initialized successfully for event tracking.")
                    
                    # Initialize enhanced duplicate checker with Redis and MISP clients
                    duplicate_checker = EnhancedDuplicateChecker(redis_client, misp_client)
                    ...
                    logger.warning("Redis client initialized but not connected. Event tracking disabled.")
                    redis_client = None
                    
            except Exception as e:
                logger.warning(f"Failed to initialize Redis client: {e}. Event tracking disabled.")
                redis_client = None
        else:
        ...
        quality_filter = EventQualityFilter()
        logger.info("🛡️  Event Quality Filter initialized - will prevent comment-only events from being published")
        
        while not is_shutdown_requested():
            try:
                process_taxii_to_misp(config, taxii_client, misp_client, stix_processor, redis_client, duplicate_checker, quality_filter)
                
                if not is_shutdown_requested():
                    logger.info(f"Sleeping for {config.SCHEDULER_INTERVAL_SECONDS} seconds before next synchronization cycle...")
