...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.MISP_DISTRIBUTION_LEVEL: int = int(os.getenv('MISP_DISTRIBUTION_LEVEL', '0'))
        ...
        self.MISP_ALERT_ON_PUBLISH: bool = self._parse_boolean(os.getenv('MISP_ALERT_ON_PUBLISH', 'false'))
        self.MISP_REQUEST_TIMEOUT: int = int(os.getenv('MISP_REQUEST_TIMEOUT', '120'))  # 120 seconds default
        
        # Processing Settings
        self.MAX_EVENTS_PER_RUN = None  # Set to None for unlimited processing
        self.SCHEDULER_INTERVAL_SECONDS: int = self._get_optional_int_env('SCHEDULER_INTERVAL_SECONDS') or 3600
        self.TEMP_DIR: str = os.getenv('TEMP_DIR', './temp')
        self.UPDATE_EXISTING_EVENTS: bool = self._parse_boolean(os.getenv('UPDATE_EXISTING_EVENTS', 'false'))
        self.COMPREHENSIVE_PROCESSING: bool = self._parse_boolean(os.getenv('COMPREHENSIVE_PROCESSING', 'false'))
        
        # Advanced TAXII Settings
        self.TAXII_REQUEST_TIMEOUT: int = int(os.getenv('TAXII_REQUEST_TIMEOUT', '300'))
        self.TAXII_MAX_OBJECTS_PER_REQUEST: int = int(os.getenv('TAXII_MAX_OBJECTS_PER_REQUEST', '1000'))
        self.TAXII_GROUPING_SEARCH_PAGES: int = int(os.getenv('TAXII_GROUPING_SEARCH_PAGES', '50'))
        self.TAXII_CHUNK_SIZE: int = int(os.getenv('TAXII_CHUNK_SIZE', '1000'))
        
        # Resource Management Settings
        self.TAXII_BATCH_SIZE: int = int(os.getenv('TAXII_BATCH_SIZE', '10000'))  # Objects per batch
        self.TAXII_REST_SECONDS: int = int(os.getenv('TAXII_REST_SECONDS', '5'))  # Rest between batches
        
        # Redis Configuration for Event Tracking
        self.REDIS_HOST: str = os.getenv('REDIS_HOST', 'localhost')
        self.REDIS_PORT: int = int(os.getenv('REDIS_PORT', '6379'))
        self.REDIS_DB: int = int(os.getenv('REDIS_DB', '1'))  # Use DB 1 for taxii2misp to avoid conflicts
        self.REDIS_PASSWORD: str = os.getenv('REDIS_PASSWORD', None)
        self.REDIS_ENABLE_TRACKING: bool = self._parse_boolean(os.getenv('REDIS_ENABLE_TRACKING', 'true'))
        self.REDIS_TTL_SECONDS: int = int(os.getenv('REDIS_TTL_SECONDS', '86400'))  # 24 hours default
    ...
    def _get_optional_int_env(self, key: str) -> int | None:
        ...
        if value:
            ...
            try:
                ...
                raise ValueError(f"Environment variable '{key}' must be an integer if set.")
        return None
