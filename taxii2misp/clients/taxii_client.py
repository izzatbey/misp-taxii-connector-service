...
class TAXIIClient:
    def __init__(self, discovery_url, username, password, verify_ssl, request_timeout=60, chunk_size=100):
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.info(f"Using collection: {self.collection.title} (ID: {self.collection.id})")
            
        except Exception as e:
            logger.error(f"Failed to initialize TAXII client: {e}")
            raise

    def get_all_objects_with_resource_management(self, redis_client=None, batch_size=10000, rest_seconds=5):
        """
        ...
        """
        logger.info(f"🚀 Starting resource-managed retrieval (batch_size={batch_size}, rest={rest_seconds}s)")
        ...
        try:
            ...
            if redis_client and redis_client.is_connected():
                try:
                    ...
                    if cached_progress:
                        last_processed_offset = int(cached_progress.get('last_offset', 0))
                        last_run_time = cached_progress.get('last_run_time', '')
                        logger.info(f"📋 Resuming GLOBAL processing from cached position: offset {last_processed_offset}")
            ...
            while True:
                ...
                logger.info(f"📦 Processing batch {batch_count + 1} (offset: {current_offset}, size: {batch_size})")
                ...
                try:
                    ...
                    try:
                        ...
                        if hasattr(response, 'as_pages'):
                            ...
                            # Skip to the correct page based on current_offset
                            current_page = current_offset // batch_size
                            page_count = 0
                            
                            for page in pages:
                                if page_count == current_page:
                                    page_envelope = page.get('envelope', page)
                                    batch_objects = page_envelope.get('objects', [])
                                    break
                                page_count += 1
                                
                            # If no more pages, we're done
                            if page_count <= current_page and not batch_objects:
                                batch_objects = []
                        else:
                        ...
                        try:
                            envelope = self.collection.get_objects(limit=batch_size * 10)  # Get more objects
                            ...
                            logger.error(f"All batch fetch methods failed: {fetch_error}")
                            batch_objects = []
                    ...
                    logger.info(f"   Rate: {len(batch_objects)/batch_time:.1f} objects/sec")
                    
                    # Log object type distribution for this batch
                    batch_types = {}
                    for obj in batch_objects:
                        obj_type = obj.get('type', 'unknown') if isinstance(obj, dict) else getattr(obj, 'type', 'unknown')
                    ...
                    if redis_client and redis_client.is_connected():
                        try:
                            progress_data = {
                                'last_offset': current_offset + len(batch_objects),  # Save next offset
                                'batch_count': batch_count + 1,
                                'total_processed': total_processed,
                                'last_run_time': datetime.now().isoformat(),
                                'timestamp': time.time()
                            }
                            redis_client.set_cached_data(cache_key, progress_data, ttl_seconds=86400)  # 24 hour TTL
                            logger.info(f"💾 Saved progress: next offset {current_offset + len(batch_objects)}")
                        except Exception as e:
                    ...
                    if rest_seconds > 0:
                        logger.info(f"😴 Resting for {rest_seconds} seconds to manage system resources...")
                    ...
                    logger.error(f"Error processing batch {batch_count + 1}: {batch_error}")
            ...
            if redis_client and redis_client.is_connected():
                try:
                    ...
                    if batch_count > 0:
                        ...
                        redis_client.set_cached_data(completion_key, completion_data, ttl_seconds=86400)
                        
                        # Clear the progress cache to start fresh next time
                        redis_client.redis_client.delete(cache_key)
                        logger.info("🧹 Cleared progress cache after successful completion - will start fresh next run")
                except Exception as e:
                    logger.warning(f"Failed to clear progress cache: {e}")
            
            logger.info(f"🎯 Resource-managed retrieval completed: {total_processed} total objects processed in {batch_count} batches")
    ...
    def _fetch_batch_with_pages(self, offset, batch_size):
        ...
        try:
            ...
            logger.debug(f"Fetching pages {start_page} to {end_page} for offset {offset}")
    ...
    def discover_available_object_types(self):
        ...
        try:
            ...
            return [
                'indicator', 'grouping', 'malware', 'threat-actor', 'attack-pattern',
                'vulnerability', 'intrusion-set', 'campaign', 'course-of-action', 
                'tool', 'identity', 'marking-definition', 'location', 'note',
                'observed-data', 'opinion', 'report', 'infrastructure'
            ]
        ...
        try:
            ...
            try:
                ...
                for page_envelope in envelope_pages:
                    ...
                    logger.debug(f"Retrieved {len(page_objects)} {object_type} objects in this page (total: {retrieved_count})")
                    
                    # Small delay to avoid overwhelming the server
                    time.sleep(0.1)
                    
            except Exception as page_error:
                logger.warning(f"Pagination failed for {object_type}, trying direct fetch: {page_error}")
                ...
                try:
                    ...
                    logger.error(f"Both pagination and direct fetch failed for {object_type}: {fallback_error}")
                    return []
            
            logger.info(f"Successfully retrieved {retrieved_count} {object_type} objects")
            return objects
    ...
    def get_all_objects_with_grouping_priority(self, max_objects=None, max_batches=None):
        """
        ...
        try:
            ...
            logger.info("🚀 Starting ENHANCED STIX object retrieval with performance optimizations...")
            ...
            grouping_objects = self._get_objects_by_type('grouping', max_objects=grouping_limit)
            grouping_time = time.time() - grouping_start
            
            logger.info(f"📦 Found {len(grouping_objects)} grouping objects in {grouping_time:.2f}s")
            
            if not grouping_objects:
                logger.warning("No grouping objects found. Falling back to direct object retrieval...")
            ...
            if referenced_ids:
                ...
                if max_batches:
                    logger.info(f"⚡ PERFORMANCE MODE: Searching for referenced objects (max {max_batches} batches)")
                else:
                    logger.info(f"🔍 Searching for all {len(referenced_ids)} referenced objects with optimizations")
                
                referenced_objects = self._search_for_referenced_objects(referenced_ids, max_batches=max_batches)
                search_time = time.time() - search_start
                
                all_objects.extend(referenced_objects)
                logger.info(f"🎯 Reference search completed in {search_time:.2f}s: found {len(referenced_objects)} objects")
            
            # Step 4: Apply max_objects limit if specified
            if max_objects and len(all_objects) > max_objects:
                logger.info(f"📊 Limiting results to {max_objects} objects (from {len(all_objects)} total)")
            ...
            logger.info(f"   🔗 Referenced: {len(referenced_objects) if 'referenced_objects' in locals() else 0} objects")
            logger.info(f"   📊 Total: {len(all_objects)} objects")
            logger.info(f"   ⚡ Performance: {len(all_objects)/total_time:.1f} objects/second")
    ...
    def get_all_objects_with_comprehensive_batching(self, max_objects=None, batch_size=5000):
        """
        ...
        try:
            start_time = time.time()
            logger.info(f"🚀 Starting COMPREHENSIVE BATCHING with {batch_size} objects per batch...")
            ...
            logger.info(f"📦 Found {len(all_groupings)} grouping objects in {grouping_time:.2f}s")
            ...
            logger.info(f"🔗 Total unique objects referenced by all groupings: {len(all_referenced_ids)}")
            ...
            if all_referenced_ids:
                search_start = time.time()
                logger.info(f"🔍 Starting comprehensive batch search for {len(all_referenced_ids)} objects in batches of {batch_size}")
                
                # Process in batches of specified size (default 5000)
                referenced_objects = self._comprehensive_batch_search(all_referenced_ids, batch_size)
                search_time = time.time() - search_start
                
                all_objects.extend(referenced_objects)
                logger.info(f"🎯 Comprehensive search completed in {search_time:.2f}s: found {len(referenced_objects)} objects")
    ...
    def _comprehensive_batch_search(self, object_ids: Set[str], batch_size: int = 5000) -> List[Dict]:
        """
        ...
        try:
            ...
            logger.info(f"🔍 COMPREHENSIVE BATCH SEARCH: Processing {len(object_id_list)} objects in {total_batches} batches of {batch_size}")
            ...
            for batch_num in range(total_batches):
                ...
                try:
                    logger.info(f"🔎 Processing batch {batch_num + 1}/{total_batches}: {len(batch_ids)} objects")
                    ...
                    if batch_objects:
                        ...
                        logger.info(f"✅ Batch {batch_num + 1}/{total_batches}: {len(batch_objects)} objects found ({batch_time:.2f}s, ETA: {eta:.1f}s)")
                    else:
            ...
            logger.info(f"   ⚡ Throughput: {len(found_objects)/total_time:.1f} objects/second")
    ...
    def _verify_bundling_completeness(self, grouping_summary: Dict, referenced_objects: List[Dict]):
        """
        ...
        """
        try:
            found_ids = {obj.get('id') for obj in referenced_objects if isinstance(obj, dict) and 'id' in obj}
            ...
            logger.info(f"   📊 Overall bundling completeness: {len(found_ids)}/{total_refs} objects ({found_ratio*100:.1f}%)")
            ...
            if found_ratio >= 0.95:
                ...
                logger.warning("   ❌ Poor bundling completeness (<80%) - some indicators may be missing")
            
        except Exception as e:
            logger.error(f"Error verifying bundling completeness: {e}")

    def _search_for_referenced_objects(self, object_ids: Set[str], max_batches: Optional[int] = None) -> List[Dict]:
        """
        ...
        try:
            ...
            if len(object_ids) > 50:
                logger.info(f"Large ID set detected ({len(object_ids)} IDs) - using optimized batch search only")
            ...
            common_types = ['indicator', 'identity', 'malware', 'threat-actor', 'attack-pattern']
            ...
            for obj_type in common_types:
                if not remaining_ids:  # Early termination if all IDs found
                    logger.info("All referenced objects found - terminating type search early")
                    break
                    
                try:
                    logger.info(f"Searching {obj_type} objects for {len(remaining_ids)} remaining IDs...")
                    
                    # Use smaller chunks for better responsiveness
                    type_objects = self._get_objects_by_type_optimized(obj_type, target_ids=remaining_ids)
                    ...
                    if type_found > 0:
                        logger.info(f"✓ Found {type_found} {obj_type} objects ({len(remaining_ids)} IDs remaining)")
                    else:
            ...
            if remaining_ids and (len(found_objects) < len(object_ids) * 0.8):
                logger.info(f"Running optimized batch search for {len(remaining_ids)} remaining IDs...")
                batch_found = self._optimized_batch_search(remaining_ids, max_batches)
                found_objects.extend(batch_found)
            
            # Remove duplicates efficiently
            unique_objects = {obj['id']: obj for obj in found_objects if isinstance(obj, dict) and 'id' in obj}
            final_objects = list(unique_objects.values())
            
            logger.info(f"Search completed: found {len(final_objects)} objects out of {len(object_ids)} requested ({len(final_objects)/len(object_ids)*100:.1f}% success rate)")
    ...
    def _optimized_batch_search(self, object_ids: Set[str], max_batches: Optional[int] = None) -> List[Dict]:
        """
        ...
        try:
            ...
            # Calculate batches
            total_batches = (len(object_id_list) + search_batch_size - 1) // search_batch_size
            ...
            if max_batches and max_batches < total_batches:
                logger.info(f"🚀 PERFORMANCE MODE: Processing {max_batches}/{total_batches} batches (batch size: {search_batch_size})")
            else:
                logger.info(f"🚀 OPTIMIZED BATCH SEARCH: Processing {total_batches} batches (batch size: {search_batch_size})")
            ...
            for batch_num in range(batches_to_process):
                ...
                try:
                    ...
                    if batch_objects:
                        ...
                        logger.info(f"⚡ Batch {batch_num + 1}/{batches_to_process}: {len(batch_objects)} objects ({batch_time:.2f}s, ETA: {eta:.1f}s)")
                    else:
                        logger.debug(f"Empty batch {batch_num + 1}/{batches_to_process}")
            ...
            total_time = time.time() - start_time
            success_rate = (successful_batches / batches_to_process) * 100 if batches_to_process > 0 else 0
            
            logger.info(f"🎯 Batch search completed: {len(found_objects)} objects in {total_time:.2f}s ({success_rate:.1f}% batch success rate)")
    ...
    def _get_objects_by_type_optimized(self, object_type: str, target_ids: Set[str] = None, max_objects: int = None) -> List[Dict]:
        """
        ...
        try:
            ...
            chunk_size = min(self.chunk_size * 2, 200)  # Double the chunk size but cap at 200
            ...
            try:
                ...
                for page_num, page_envelope in enumerate(envelope_pages):
                    ...
                    if target_ids and found_target_count >= len(target_ids):
                        logger.debug(f"All target {object_type} objects found - terminating early")
                        break
                    ...
                    if target_ids:
                        page_target_count = sum(1 for obj in page_objects if isinstance(obj, dict) and obj.get('id') in target_ids)
                        found_target_count += page_target_count
                        
                        if page_target_count > 0:
                            logger.debug(f"Page {page_num + 1}: found {page_target_count} target {object_type} objects")
    ...
    def extract_grouping_objects(self, stix_objects):
        ...
        try:
            ...
            logger.info(f"Processing {len(stix_objects)} STIX objects for grouping extraction")
    ...
    def get_objects_for_testing(self, max_objects=50, max_batches=2):
        ...
        """
        logger.info(f"Running test mode: max {max_objects} objects, max {max_batches} batches")
