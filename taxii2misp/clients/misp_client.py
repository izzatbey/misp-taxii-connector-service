...
class MISPClient:
    ...
    def create_simple_client(
        ...
        try:
            ...
            logger.info(
                "Creating simple MISP client bypassing version compatibility checks..."
            )
            ...
            def _prepare_request(method, url, data=None, **kwargs):
                ...
                return instance.misp._PyMISP__session.request(
                    method, url, timeout=instance.misp.timeout, **kwargs
                )
            ...
            def _check_json_response(response):
                ...
                try:
                    if response.status_code in [
                        200,
                        201,
                    ]:  # Include 201 for successful creation
            ...
            logger.info(
                "Simple MISP client created successfully (bypassing version checks)"
            )
            return instance
        ...
        try:
            ...
            try:
                ...
                if "version" in version_info:
                    logger.info(
                        f"Connected to MISP (version {version_info['version']}) at {self.misp_url}"
                    )
                else:
                    logger.info(
                        f"Connected to MISP (version unknown) at {self.misp_url}"
                    )
            except Exception as version_error:
                logger.warning(
                    f"Version check failed but connection may still work: {version_error}"
                )
            ...
            try:
                ...
                logger.warning(
                    "Fallback MISP client initialized - some features may be limited"
                )

            except Exception as fallback_error:
                logger.error(
                    f"All MISP client initialization methods failed: {fallback_error}"
                )
                raise RuntimeError(
                    f"Could not initialize MISP client: {fallback_error}"
                )

    def add_event(self, event, publish: bool = True):
        """Add a MISPEvent to MISP following PyMISP api.py pattern.

        When publish=True (the default), the event is marked as published
        BEFORE add_event is called, so PyMISP issues a single REST call
        that creates + publishes the event at once. This halves the
        MISP network round-trips per event compared to the previous
        add-then-publish flow.

        For backward compatibility, callers may still invoke
        ``publish_event(event_id)`` afterwards — that becomes a no-op
        if the event is already published (MISP returns a benign
        ...
        try:
            ...
            logger.debug(f"Adding event to MISP (publish={publish}): {event.info}")

            # Mark as published in-process before the REST call so PyMISP
            # does it in a single round-trip. Setting .published on a
            # MISPEvent updates the underlying dict and is reflected in
            # the serialized payload.
            if publish:
                try:
                    event.published = True
                except Exception:
                    # Some MISPEvent variants guard this attribute; if
                    # we can't set it, the explicit publish_event()
                    # call from the caller will handle it. Not fatal.
                    logger.debug(
                        "Could not set event.published in-process; caller will publish."
                    )

            # Handle fallback client with custom implementation
            if (
                hasattr(self.misp, "_is_fallback_client")
                and self.misp._is_fallback_client
            ):
    ...
    def _add_event_fallback(self, event):
        ...
        try:
                        ...
                        if hasattr(attr, "to_dict"):
                            ...
                            attr_dict = {
                                ...
                                "distribution": getattr(
                                    attr, "distribution", self.distribution
                                ),
                            }
            ...
            if isinstance(response_data, dict) and "errors" not in response_data:
                ...
                logger.error(
                    f"Failed to add event via fallback method: {response_data}"
                )
                return None
    ...
    def _serialize_misp_objects(self, obj):
        ...
        if hasattr(obj, "to_dict"):
            ...
            return {
                k: self._serialize_misp_objects(v)
    ...
    def add_event_with_deduplication(self, event, similarity_threshold=0.7):
        ...
        try:
            ...
            is_duplicate, existing_event = self._is_duplicate_event(
                event, similarity_threshold
            )

            if is_duplicate:
                existing_id = (
                    existing_event.get("Event", {}).get("id", "Unknown")
                    if existing_event
                    else "Unknown"
                )
                logger.warning(
                    f"🔄 DUPLICATE DETECTED: Event '{event_info}' is similar to existing event ID {existing_id} - SKIPPING"
                )
            ...
            logger.info(
                f"✅ No duplicate found for '{event_info}' - proceeding with creation"
            )
            ...
            if response and "Event" in response:
                ...
                logger.info(
                    f"🎉 NEW EVENT CREATED: '{event_info}' with ID {new_event_id}"
                )
            else:
    ...
    def _is_duplicate_event(self, new_event, similarity_threshold=0.7):
        ...
        try:
            ...
            if (
                ...
                logger.debug(
                    "Fallback client detected - using simplified deduplication check"
                )
                return self._is_duplicate_event_fallback(
                    new_event, similarity_threshold
                )
            ...
            if hasattr(new_event, "info") and new_event.info:
                try:
                    search_results = self.misp.search(
                        eventinfo=new_event.info, pythonify=False
                    )
                    if (
                        search_results
                        ...
                        for result in search_results:
                            if isinstance(result, dict) and "Event" in result:
                                ...
                                if self._has_similar_attributes(
                                    new_event, existing_event, similarity_threshold
                                ):
            ...
            try:
                ...
                for existing_event in recent_events:
                    if self._has_similar_attributes(
                        ...
                        logger.debug(
                            f"Found similar recent event: {existing_event.get('Event', {}).get('id', 'Unknown')}"
                        )
                        return True, existing_event
    ...
    def _has_similar_attributes(
        self, new_event, existing_event_response, threshold=0.7
    ):
        """
        ...
        try:
            ...
            for new_attr in new_attributes:
                ...
                new_attr_hash = hashlib.md5(
                    str(new_attr).lower().strip().encode()
                ).hexdigest()

                for existing_attr in existing_values:
                    existing_attr_hash = hashlib.md5(
                        str(existing_attr).lower().strip().encode()
                    ).hexdigest()
            ...
            similarity_ratio = (
                matching_attributes / total_new_attributes
                if total_new_attributes > 0
                else 0
            )

            logger.debug(
                f"Attribute similarity: {similarity_ratio:.2f} ({matching_attributes}/{total_new_attributes})"
            )
    ...
    def _is_duplicate_event_fallback(self, new_event, similarity_threshold=0.7):
        ...
        try:
            logger.info(
                f"Checking for duplicates of event: {getattr(new_event, 'info', 'Unknown')}"
            )
            ...
            if hasattr(new_event, "info") and new_event.info:
                try:
                    ...
                    if isinstance(response_data, list):
                        ...
                        isinstance(response_data, dict) and "response" in response_data
                    ):
                        # Response has nested structure
                        events_list = response_data["response"]
                    else:
                        logger.debug(
                            f"Unexpected response format from events/index: {type(response_data)}"
                        )
                        events_list = []

                    logger.debug(
                        f"Found {len(events_list)} events to check for duplicates"
                    )
                    ...
                    for event_item in events_list[
                        ...
                        try:
                            ...
                            if isinstance(event_item, dict):
                                ...
                                existing_info = (
                                    existing_event_data.get("info", "").strip().lower()
                                )

                                # Check for title match
                                if existing_info == new_info:
                                    logger.info(
                                        f"Found exact title match for '{new_event.info}'"
                                    )
                                    ...
                                    if self._has_similar_attributes_fallback(
                                        new_event,
                                        existing_event_data,
                                        similarity_threshold,
                                    ):
                                        logger.info(
                                            f"Event is duplicate of existing event ID: {existing_event_data.get('id', 'Unknown')}"
                                        )
                                        return True, existing_event
    ...
    def _has_similar_attributes_fallback(
        self, new_event, existing_event_data, threshold=0.7
    ):
        """
        ...
        try:
            ...
            try:
                ...
                similarity_ratio = (
                    matching_attributes / len(new_attributes) if new_attributes else 0
                )

                logger.debug(
                    f"Attribute similarity: {similarity_ratio:.2f} ({matching_attributes}/{len(new_attributes)})"
                )
    ...
    def publish_event(self, event_id, alert=False):
        ...
        Safe to call when the event is already published (which is the
        common case now that add_event() pre-sets event.published=True).
        In that scenario, MISP returns a benign "already published"
        response or a 400 with that exact phrase; we treat both as
        success rather than reporting a false failure to the caller.
        """
        try:
            logger.debug(f"Publishing MISP event {event_id} (alert={alert})")
            ...
            # Treat "already published" as success — it just means
            # add_event() pre-published the event, and the redundant
            # publish call here is a no-op.
            if isinstance(response, dict) and "errors" in response:
                errs = response["errors"]
                if isinstance(errs, (list, tuple)) and any(
                    "already" in str(e).lower() and "publish" in str(e).lower()
                    for e in errs
                ):
                    logger.debug(f"Event {event_id} already published; no-op.")
                    return True
    ...
    def _publish_event_fallback(self, event_id, alert=False):
        ...
        try:
            ...
            if isinstance(response_data, dict) and "errors" not in response_data:
                logger.info(
                    f"Event {event_id} published successfully via fallback method"
                )
                return True
            else:
                logger.error(
                    f"Failed to publish event {event_id} via fallback method: {response_data}"
                )
                return False
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(events, (list, dict)):
                        ...
                        return (
                            False,
                            f"Unexpected response from events API: {type(events)}",
                        )
                except Exception as fallback_error:
    ...
    def search_events_by_info(self, event_info: str, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            response = self.misp.search(
                controller="events", eventinfo=event_info, limit=limit, pythonify=False
            )
    ...
    def _search_events_fallback(self, event_info: str, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            params = {"searcheventinfo": event_info, "limit": limit}
            ...
            for event_data in events:
                ...
                if isinstance(event, dict):
                    ...
                    if existing_info.strip().lower() == event_info.strip().lower():
                        logger.info(
                            f"Found existing event with matching info: '{event_info}' (ID: {event.get('id', 'unknown')})"
                        )
                        return event
    ...
    def search_events_by_uuid(self, event_uuid: str) -> Optional[Dict]:
        ...
        try:
            ...
            response = self.misp.search(
                controller="events", uuid=event_uuid, pythonify=False
            )
