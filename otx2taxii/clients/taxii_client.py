...
class TAXIIClient:
    ...
    def __init__(self, taxii_url: str, username: str, password: str, verify_ssl: bool = False,
                 redis_host: str = 'localhost', redis_port: int = 6379, redis_db: int = 0,
                 redis_password: str | None = None, cache_ttl_seconds: int = 3600):
        ...
        self.redis_client_instance = RedisClient(redis_host, redis_port, redis_db, redis_password)
    ...
    def connect_to_server(self) -> Server:
        ...
        try:
            ...
            logger.critical(f"Access Denied to TAXII Server: {ae}. Check credentials. Exiting.")
            raise
        except requests.exceptions.RequestException as req_e:
            logger.critical(f"Error connecting to TAXII server: {req_e}. Exiting.")
            raise
        except Exception as e:
            logger.critical(f"Unexpected error during TAXII server connection: {e}. Exiting.")
            raise
    ...
    def get_default_collection(self) -> Collection:
        ...
        logger.info(f"Selected TAXII Collection: {self._collection.title} (ID: {self._collection.id})")
        return self._collection

    def _fetch_all_stix_ids_from_taxii(self, max_objects_for_test: int | None = None) -> set[str]:
        """
        Internal method to fetch all STIX Object IDs directly from the TAXII server.
        """
        logger.info(f"TAXIIClient._fetch_all_stix_ids_from_taxii received max_objects_for_test: {max_objects_for_test}")

        fetched_ids = set()
        if not self._collection:
            logger.warning("TAXII collection not initialized for direct fetch. Cannot retrieve existing IDs.")
            return fetched_ids

        logger.info(f"Fetching ALL STIX Object IDs directly from TAXII Collection '{self._collection.title}'...")
        try:
            ...
            for envelope in as_pages(
                ...
                if isinstance(envelope, TAXIIStatus):
                    ...
                    logger.info("TAXII collection returned an empty dictionary page. Assuming no more objects.")
                    break
                else:
                    logger.warning(
                ...
                if not current_objects:
                    logger.info(f"Page {page_counter} returned no objects. Assuming end of collection.")
                    break

                for obj in current_objects:
                    fetched_ids.add(obj['id'])

                logger.info(f"[*] Page {page_counter}: Added {len(current_objects)} objects. Total collected so far: {len(fetched_ids)}")

                if max_objects_for_test is not None and len(fetched_ids) >= max_objects_for_test:
                    logger.info(f"Reached test limit of {max_objects_for_test} objects. Stopping fetch.")
                    break

        except AccessError as ae:
            logger.error(f"[✘] Access Denied when getting existing IDs from TAXII. Error: {ae}")
        except requests.exceptions.RequestException as req_e:
            logger.error(f"[✘] HTTP/Network Error getting existing IDs from TAXII: {req_e}")
        except Exception as e:
            logger.error(f"[✘] Failed to get existing IDs directly from TAXII: {e}", exc_info=True)

        logger.info(f"[✔] Successfully retrieved {len(fetched_ids)} existing STIX Object IDs directly from TAXII.")
        return fetched_ids

    def get_existing_stix_ids(self, max_objects_for_test: int | None = None) -> set[str]:
        """
        ...
        existing_ids = set()
        if not self._collection:
            logger.warning("TAXII collection not initialized. Cannot retrieve existing IDs.")
            return existing_ids
        ...
        if self.redis_client:
            try:
                ...
                if cached_ids:
                    logger.info(f"Retrieved {len(cached_ids)} STIX IDs from Redis cache (key: {cache_key}).")
                    ...
                    logger.info(f"No STIX IDs found in Redis cache or cache expired for key: {cache_key}. Fetching from TAXII.")
            except Exception as e:
                logger.error(f"Error accessing Redis cache for TAXII IDs: {e}. Fetching from TAXII instead.", exc_info=True)
                # Fallback to direct fetch if Redis fails
        else:
            logger.warning("Redis client not available. Fetching all STIX IDs directly from TAXII (no caching).")

        # --- NEW DIAGNOSTIC LOG (ADDED HERE) ---
        logger.info(f"TAXIIClient.get_existing_stix_ids is about to call _fetch_all_stix_ids_from_taxii with max_objects_for_test={max_objects_for_test}")
        # --- END NEW DIAGNOSTIC LOG ---

        # If not in cache or Redis not available, fetch directly from TAXII
        existing_ids = self._fetch_all_stix_ids_from_taxii(max_objects_for_test=max_objects_for_test)
        ...
        if self.redis_client and existing_ids and max_objects_for_test is None:
            try:
                ...
                logger.info(f"Stored {len(existing_ids)} STIX IDs in Redis cache (key: {cache_key}) with TTL: {self.cache_ttl_seconds}s.")
            except Exception as e:
                logger.error(f"Failed to store STIX IDs in Redis cache: {e}", exc_info=True)
            ...
            logger.info(f"Skipping cache for TAXII IDs because max_objects_for_test was set ({max_objects_for_test}).")
    ...
    def add_stix_ids_to_cache(self, new_ids: set[str]) -> None:
        ...
        try:
            if new_ids:
                ...
                # Refresh TTL
                self.redis_client.expire(cache_key, self.cache_ttl_seconds)
                logger.info(f"Added {len(new_ids)} new STIX IDs to Redis cache (key: {cache_key}).")
        except Exception as e:
            logger.error(f"Failed to add new STIX IDs to Redis cache: {e}", exc_info=True)
    ...
    def check_stix_ids_existence(self, stix_ids: set[str]) -> tuple[set[str], set[str]]:
        ...
        try:
            ...
            logger.info(f"ID existence check: {len(existing_ids)} existing, {len(new_ids)} new")
            return existing_ids, new_ids
            
        except Exception as e:
            logger.error(f"Failed to check STIX IDs existence in Redis cache: {e}", exc_info=True)
            return set(), stix_ids
    ...
    def refresh_cache_from_taxii(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if not force and self.redis_client.exists(cache_key):
                ...
                if ttl > 0:
                    logger.info(f"Cache is still valid for {ttl} seconds. Skipping refresh.")
                    return False
    ...
    def get_first_page_of_stix_objects(self, page_limit: int = 50) -> list[dict]:
        ...
        logger.info(f"Retrieving first page of STIX Objects from TAXII Collection '{self._collection.title}' (Limit: {page_limit})...")
        try:
            get_objects_params = {
                'limit': page_limit
            }
            ...
            if isinstance(objects_response, TAXIIStatus):
                ...
                logger.info("TAXII collection returned an empty dictionary for the first page.")
                return []
            else:
                logger.warning(
            ...
            logger.error(f"[✘] Access Denied when getting first page objects. Error: {ae}")
        except requests.exceptions.RequestException as req_e:
            logger.error(f"[✘] HTTP/Network Error getting first page objects from TAXII: {req_e}")
        except Exception as e:
    ...
    def add_stix_bundle(self, bundle: dict) -> bool:
        ...
        logger.info(f"Attempting to push STIX Bundle with {num_objects} objects to TAXII...")
        try:
            ...
            if isinstance(push_response, TAXIIStatus):
                logger.info(f"[✔] Bundle pushed successfully. Status: {push_response.status}")
                ...
                if self.redis_client and self._collection:
                    ...
                    if hasattr(push_response, 'success_count') and push_response.success_count > 0:
                        ...
                        if hasattr(push_response, 'failed') and push_response.failed:
                            ...
                            failed_ids = {f_obj.get('id') for f_obj in push_response.failed if f_obj.get('id')}
                            # Add IDs of non-failed objects to successful set
                            successful_ids = {obj.get('id') for obj in bundle_objects if obj.get('id') and obj.get('id') not in failed_ids}
                        else:
                            # No failures reported, all objects successful
                            successful_ids = {obj.get('id') for obj in bundle_objects if obj.get('id')}
                    
                    # Update cache with successful IDs
                    if successful_ids:
                        self.add_stix_ids_to_cache(successful_ids)
                        logger.info(f"Added {len(successful_ids)} new STIX IDs to Redis cache.")
                    
                # Log detailed response information
                if hasattr(push_response, 'success_count'):
                    logger.info(f"Successfully sent: {push_response.success_count} objects.")
                if hasattr(push_response, 'failed_count'):
                    logger.warning(f"Failed to send: {push_response.failed_count} objects.")
                if hasattr(push_response, 'failed') and push_response.failed:
                ...
                logger.error(f"[✘] Unexpected response type from TAXII server: {type(push_response)}. Response: {push_response}")
                return False
        except AccessError:
            logger.error("[✘] Access Denied when pushing bundle to TAXII server. Check credentials.")
            return False
        except requests.exceptions.RequestException as req_e:
            logger.error(f"[✘] Network or HTTP Error when pushing bundle: {req_e}")
            return False
        except Exception as e:
            logger.error(f"[✘] Failed to push Bundle to TAXII Server: {e}", exc_info=True)
            return False
    def filter_new_stix_objects(self, stix_objects: list[dict]) -> list[dict]:
        ...
        groupings = [obj for obj in stix_objects if obj.get('type') in ('grouping', 'report', 'campaign')]
            ...
            if group_id in existing_ids:
                ...
                if 'object_refs' in group:
                    ...
                    for ref in group.get('object_refs', []):
                        if ref in new_ids:
                            logger.info(f"Grouping {group_id} references updated object {ref} - will update grouping")
        ...
        new_objects = [obj for obj in stix_objects if obj.get('id') in new_ids or 
                       (obj.get('type') in ('grouping', 'report', 'campaign') and obj.get('id') in updated_groupings)]
        
        logger.info(f"Filtered {len(stix_objects)} objects: {len(new_objects)} new/updated, {len(existing_ids)} duplicates")
        if updated_groupings:
            logger.info(f"Included {len(updated_groupings)} existing groupings that reference updated objects")
    ...
    def pre_validate_bundle_objects(self, bundle: dict) -> dict:
        ...
        logger.info(f"Bundle pre-validation: {original_count} original objects, {len(new_objects)} new/updated objects")
    ...
    def get_missing_stix_ids(self, required_ids: set[str]) -> set[str]:
        ...
        logger.info(f"Missing ID check: {len(missing_ids)} missing out of {len(required_ids)} total")
