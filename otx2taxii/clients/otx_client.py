...
class OTXClient:
    ...
    def __init__(self, api_key: str,
                 redis_host: str = 'localhost', redis_port: int = 6379, redis_db: int = 0, redis_password: str | None = None):
        if not api_key:
            raise ValueError("OTX API Key is required.")
        
        otx_cache_dir = os.getenv("OTX_CACHE_DIR", os.path.expanduser("~/.otx_cache_data"))
        self.otx = OTXv2Cached(api_key, cache_dir=otx_cache_dir)
        logger.info(f"OTX Client initialized with OTXv2Cached. Cache directory: {otx_cache_dir}")
    ...
    def _get_last_fetched_timestamp(self) -> datetime.datetime | None:
        ...
        if self.redis_client:
            try:
                ...
                if timestamp_str:
                    dt = datetime.datetime.fromisoformat(timestamp_str).replace(tzinfo=pytz.UTC)
                    logger.info(f"Retrieved application's last fetched timestamp from Redis: {dt}")
                    return dt
            except Exception as e:
                logger.error(f"Error retrieving application's last fetched timestamp from Redis: {e}", exc_info=True)
        return None
    ...
    def _set_last_fetched_timestamp(self, timestamp: datetime.datetime):
        ...
        if self.redis_client:
            try:
                ...
                logger.info(f"Stored application's new last fetched timestamp in Redis: {timestamp}")
            except Exception as e:
                logger.error(f"Error storing application's last fetched timestamp in Redis: {e}", exc_info=True)

    def get_all_subscribed_pulses(self, author_name: str | None = None, max_pulses: int | None = None) -> list[dict]:
        """
        ...
        if author_name is None:
            logger.info("No author specified - will retrieve pulses from ALL subscribed authors")
        else:
        ...
        logger.info("Starting OTXv2Cached update to fetch new/modified pulses into local cache...")
        try:
            self.otx.update() 
            logger.info("OTXv2Cached update completed. Data should now be in local cache.")
            ...
            if author_name is None:
                ...
                for pulse in self.otx.getall(iter=True, limit=max_pulses):
                    if max_pulses is not None and processed_pulses_count >= max_pulses:
                        logger.info(f"Reached test limit of {max_pulses} pulses from cache. Stopping iteration.")
                        break
                    ...
                    if processed_pulses_count % 100 == 0:
                        logger.info(f"Processed {processed_pulses_count} pulses so far (from all authors)...")
            else:
                # Filter by specific author
                logger.info(f"Filtering pulses by author: {author_name}")
                for pulse in self.otx.getall(iter=True, author_name=author_name, limit=max_pulses):
                    if max_pulses is not None and processed_pulses_count >= max_pulses:
                        logger.info(f"Reached test limit of {max_pulses} pulses from cache. Stopping iteration.")
                        break
                    ...
                    if pulse_author != author_name:
                        logger.debug(f"Skipping pulse {pulse.get('id')} from author '{pulse_author}' (not {author_name})")
                        continue
                    ...
                    # Log every 100 pulses for progress tracking
                    if processed_pulses_count % 100 == 0:
                        logger.info(f"Processed {processed_pulses_count} {author_name} pulses so far...")

            if all_fetched_pulses:
                if author_name is None:
                    logger.info(f"Successfully retrieved {len(all_fetched_pulses)} pulses from OTXv2Cached's local cache (from all subscribed authors).")
                    ...
                    logger.info(f"Successfully retrieved {len(all_fetched_pulses)} pulses from OTXv2Cached's local cache (filtered by {author_name}).")
                ...
                for pulse in all_fetched_pulses:
                    ...
                    if modified_str:
                        try:
                            current_pulse_modified_dt = datetime.datetime.fromisoformat(modified_str).replace(tzinfo=pytz.UTC)
                            if latest_modified_time_in_batch is None or current_pulse_modified_dt > latest_modified_time_in_batch:
                                latest_modified_time_in_batch = current_pulse_modified_dt
                        except ValueError:
                            logger.warning(f"Could not parse 'modified' timestamp '{modified_str}' for pulse {pulse.get('id')}. Skipping for timestamp update.")

                if latest_modified_time_in_batch:
                    self._set_last_fetched_timestamp(latest_modified_time_in_batch + datetime.timedelta(seconds=1))
                else:
                    logger.info("No pulses with valid 'modified' timestamps were found to update the application's last fetch time.")
            else:
                if author_name is None:
                    logger.info("No new or updated pulses found from any subscribed author in OTXv2Cached's local cache after update.")
                else:
                    logger.info(f"No new or updated pulses by '{author_name}' found in OTXv2Cached's local cache after update.")

            return all_fetched_pulses

        except requests.exceptions.Timeout as e:
            logger.error(f"OTX API update timed out after 30 seconds: {e}. Check network connectivity or OTX API status.", exc_info=True)
            return []
        except requests.exceptions.ConnectionError as e:
            logger.error(f"Failed to connect to OTX API during update: {e}. Check network connectivity and OTX API endpoint.", exc_info=True)
            return []
        except requests.exceptions.RequestException as e:
            logger.error(f"An HTTP error occurred during OTXv2Cached update: {e}", exc_info=True)
            return []
        except Exception as e:
            logger.error(f"An unexpected error occurred during OTXv2Cached update: {e}", exc_info=True)
            return []

    def get_pulses_by_multiple_authors(self, author_names: list[str], max_pulses: int | None = None) -> list[dict]:
        """
        ...
        for author_name in author_names:
            ...
            author_pulses = self.get_all_subscribed_pulses(author_name=author_name, max_pulses=max_pulses)
    ...
    def get_alienvault_pulses_only(self, max_pulses: int | None = None) -> list[dict]:
        ...
        """
        return self.get_all_subscribed_pulses(author_name="AlienVault", max_pulses=max_pulses)
        ...
        try:
            ...
            for pulse in self.otx.getall(iter=True, limit=max_pulses):
                ...
                if processed_count % 500 == 0:
                    logger.info(f"Analyzed {processed_count} pulses for author statistics...")
            
            # Sort by pulse count (descending)
            sorted_stats = dict(sorted(author_stats.items(), key=lambda x: x[1], reverse=True))
    ...
    def get_pulse_indicators(self, pulse_id: str) -> list[dict]:
        ...
        try:
            ...
            if pulse_details and 'indicators' in pulse_details:
                logger.info(f"Retrieved {len(pulse_details['indicators'])} indicators for pulse ID: {pulse_id}.")
                return pulse_details['indicators']
            else:
                logger.warning(f"No indicators found or pulse details incomplete for ID: {pulse_id}.")
                return []
        except requests.exceptions.Timeout as e:
            logger.error(f"OTX API request for pulse {pulse_id} details timed out after 30 seconds: {e}.", exc_info=True)
            return []
        except requests.exceptions.ConnectionError as e:
            logger.error(f"Failed to connect to OTX API for pulse {pulse_id} details: {e}.", exc_info=True)
            return []
        except requests.exceptions.RequestException as e:
            logger.error(f"An HTTP error occurred retrieving pulse {pulse_id} details: {e}", exc_info=True)
            return []
        except Exception as e:
            logger.error(f"An unexpected error occurred while retrieving indicators for pulse {pulse_id}: {e}", exc_info=True)
            return []
    ...
    def _get_cached_pulse(self, pulse_id: str) -> dict | None:
        ...
        try:
            ...
            logger.error(f"Error retrieving cached pulse from Redis: {e}", exc_info=True)
            return None
    ...
    def _get_cached_pulses(self) -> list[dict]:
        ...
        try:
            ...
            logger.error(f"Error retrieving cached pulses from Redis: {e}", exc_info=True)
            return []
    ...
    def get_cached_stix_uuid(self, stix_id: str) -> str | None:
        ...
        try:
            ...
            logger.error(f"Error retrieving cached STIX UUID from Redis: {e}", exc_info=True)
            return None
    ...
    def get_pulse_stix_uuids(self, pulse_id: str) -> set:
        ...
        try:
            ...
            logger.error(f"Error retrieving pulse STIX UUIDs from Redis: {e}", exc_info=True)
            return set()
    ...
    def check_pulse_changed(self, pulse_id: str, new_indicators: list) -> bool:
        ...
        try:
            ...
            if not cached_pulse or 'indicators' not in cached_pulse:
                logger.info(f"Pulse {pulse_id} not in cache or missing indicators - treating as new/changed")
                return True
                
            if len(cached_pulse['indicators']) != len(new_indicators):
                logger.info(f"Pulse {pulse_id} indicator count changed from {len(cached_pulse['indicators'])} to {len(new_indicators)}")
                return True
            
            cached_ids = {ind.get('id') for ind in cached_pulse['indicators'] if 'id' in ind}
